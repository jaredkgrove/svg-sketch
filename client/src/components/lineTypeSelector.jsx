...
import polylineImage from '../images/Polyline.PNG'
 const LineTypeSelector = (props) => {
    ...
    return (
        <LineTypeWrapper>
            <LineTypeSvg  selected={isCurrentSetting('Circle')} onClick={() => handleClick('Circle')} image={circleImage} viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="40" fill="hsl(207, 20%, 80%)" stroke="hsl(207, 20%, 25%)" stroke-width="10"/>
            </LineTypeSvg>
            <LineTypeSvg selected={isCurrentSetting('Line')} onClick={() => handleClick('Line')}image={lineImage} viewBox="0 0 100 100">
                <line x1="10" y1="10" x2="90" y2="90" stroke="hsl(207, 20%, 25%)" stroke-width="10" />
            </LineTypeSvg> 
            <LineTypeSvg selected={isCurrentSetting('Rectangle')} onClick={() => handleClick('Rectangle')} image={rectangleImage} viewBox="0 0 100 100">
                <rect x="10" y="15" width="80" height="70" fill="hsl(207, 20%, 80%)" stroke="hsl(207, 20%, 25%)" stroke-width="10" />
            </LineTypeSvg>
            <LineTypeSvg selected={isCurrentSetting('Polyline')} onClick={() => handleClick('Polyline')} image={polylineImage} viewBox="0 0 100 100">
