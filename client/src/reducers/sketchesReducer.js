const sketchReducer = (state = [], action) => {
    switch(action.type) {
      ...
      case 'CREATE_SKETCH':
        return [...state, {id: action.payload.id, name: action.payload.name, url:`/sketches/${action.payload.id}`, lastUpdated: action.payload.lastUpdated, public: action.payload.public}]
      case 'DELETE_SKETCH':
        let deleteIndex = state.findIndex(sketch => parseInt(sketch.id) === action.payload)
